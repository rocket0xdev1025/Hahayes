!(function ($, _undefined) {
  ...
  $ush.toArray = function (data) {
    if (
      ["string", "number", "bigint", "boolean", "symbol", "function"].includes(
        typeof data
      )
    ) {
      return [data];
  ...
  $ush.mixinEvents = {
    ...
    trigger: function (eventType, extraParams) {
      ...
      let args = arguments,
        params =
          args.length > 2 || !Array.isArray(extraParams)
            ? [].slice.call(args, 1)
            : extraParams;
...
$us.mixins.Events = {
  ...
  trigger: function (eventType, extraParameters) {
    ...
    var args = arguments,
      params =
        args.length > 2 || !Array.isArray(extraParameters)
    ...
    _events: {
      scroll: function () {
        ...
        self.$topLink.toggleClass(
          "active",
          scrollTop >= (self.winHeight * self.options.backToTopDisplay) / 100
        );
        if (self.isAndroid) {
          if (self.pid) {
      ...
      toggleClassIsSticky: function () {
        ...
        self.$stickySections.each(function (_, section) {
          ...
          $section.toggleClass(
            "is_sticky",
            parseInt(offsetTop) === 0 && $section.css("position") == "sticky"
          );
        });
      },
    },
...
!(function ($, undefined) {
  ...
  function usCollapsibleContent(container) {
    const self = this;
    self._events = { showContent: self.showContent.bind(self) };
  ...
  usCollapsibleContent.prototype = {
    setHeight: function () {
      const self = this;
      let collapsedHeight = self.$firstElement
        .css("height", self.collapsedHeight)
        .height();
      ...
      $ush.timeout(() => {
        ...
        if ($.isMobile && !$ush.isNodeInViewport(self.$container[0])) {
          $us.$htmlBody
            .stop(!0, !1)
            .scrollTop(
              self.$container.offset().top - $us.header.getCurrentHeight(!0)
            );
        }
      }, 1);
    },
  ...
  USScroll.prototype = {
    _countPosition: function (hash) {
      ...
      if (
        ...
        targetTop =
          $us.$body.outerHeight(!0) + (targetTop - $us.$window.scrollTop());
      }
    ...
    attach: function (anchors) {
      ...
      $anchors.each(function (index, anchor) {
        ...
        if (
          ...
          (hasPageId &&
            href.indexOf((_location.search.match(patternPageId) || [])[1]) ==
              -1) ||
        ...
        if (hash != "" && hash != "#") {
          ...
          if (
            ...
            self.blocks[hash].buttons = (self.blocks[hash].buttons || $()).add(
              $anchor
            );
          }
        }
    ...
    if (carouselOptions.slideBy == "page") {
      if ($(".wpb_row:first", $carouselContainer).length) {
        $.each(carouselOptions.responsive, (_, options) => {
          $.extend(options, { items: 1, autoWidth: !1 });
        });
      }
    }
    if ($us.usbPreview()) {
      ...
      $carouselContainer.one("initialized.owl.carousel", () => {
        ...
        $ush.timeout(() => {
          $(".owl-dots *, .owl-prev, .owl-next", $carouselContainer).addClass(
            "usb_skip_elmSelected"
          );
        }, 1);
      });
    ...
    $carouselContainer.one("initialized.owl.carousel", () => {
      $("[data-content-height]", $carouselContainer).each((_, node) => {
        ...
        if ($ush.isUndefined(usCollapsibleContent)) {
          usCollapsibleContent = $node
            .usCollapsibleContent()
            .data("usCollapsibleContent");
        }
  ...
  $us.WGrid.prototype = {
    init: function (container, options) {
      ...
      if (this.$container.hasClass("with_isotope")) {
            ...
            this.$list.on(
              ...
              function () {
                if (_window.USAnimate) {
                  $(".w-grid-item.off_autostart", this.$list).removeClass(
                    "off_autostart"
                  );
        ...
        self.$container.on(
          "usbReloadIsotopeLayout",
          self._events.usbReloadIsotopeLayout
        );
      ...
      this.$list.on(
        "click",
        "[ref=magnificPopup]",
        self._events.initMagnificPopup
      );
    },
    _updateState: function (e, queryString, page, gridFilter) {
      ...
      if (
        ...
        (!$container.is(":visible") && !$container.hasClass("hidden"))
      ) {
        return;
      }
      page = page || 1;
            ...
            function () {
              ...
              if (this.paginationType == "ajax") {
                ...
                if (
                  ...
                  this.$loadmore.removeClass("done").removeClass("loading");
                }
                if (this.infiniteScroll) {
                  $us.waypoints.add(
                    this.$loadmore,
...
!(function ($, undefined) {
  ...
  function USHeader(settings) {
    ...
    self.$places = { hidden: $(".l-subheader.for_hidden", self.$container) };
  ...
  $.extend(USHeader.prototype, $us.mixins.Events, {
    ...
    currentStateIs: function (state) {
      ...
      return (
        state &&
        ["default"].concat(Object.keys(self.breakpoints)).indexOf(state) !==
          -1 &&
        self.state === state
      );
    },
    ...
    isStickyAutoHideEnabled: function () {
      ...
      return (
        ...
        (((self.settings[self.state] || {}).options || {}).sticky_auto_hide ||
          !1)
      );
    },
    isSticky: function () {
    ...
    _isVerticalScrollable: function () {
      ...
      if (
        (self.currentStateIs("default") || self.currentStateIs("laptops")) &&
        self.isFixed()
      ) {
    ...
    _events: {
      ...
      scroll: function () {
        ...
        var headerAttachedFirstSection =
          ["bottom", "below"].indexOf(self.getHeaderInitialPos()) !== -1;
        if (
          self.isHorizontal() &&
          (headerAbovePosition ||
        ...
        if (
          ...
          if (documentHeight > headerHeight) {
            var canvasHeight =
                parseInt($us.canvas.winHeight) + self.canvasOffset,
            ...
            if (self.bodyHeight > headerHeight) {
              if (scrollTop < self._headerScrollRange[0]) {
                ...
                self._headerScrollRange[1] =
                  self._headerScrollRange[0] + scrollRangeDiff;
                cssProps = { position: "fixed", top: self.adminBarHeight };
              } else if (
                ...
                self._headerScrollRange[0] =
                  self._headerScrollRange[1] - scrollRangeDiff;
                cssProps = {
                  position: "fixed",
              ...
              cssProps = { position: "absolute", top: self.adminBarHeight };
            }
            if (cssProps) {
    ...
    init: function (container, options) {
      ...
      this._events = {
        ...
        keyUpHandler: function (e) {
          ...
          if (e.keyCode == 9) {
            $item.addClass("opened").siblings().removeClass("opened");
          }
          if (e.keyCode == 27) {
            this.$items.removeClass("opened");
            $target.blur();
    ...
    show: function (e) {
      ...
      if (self.triggerType == "load" && !$us.usbPreview()) {
        ...
        if (uniqueId) {
          ...
          var daysUntilNextShow = $ush.parseFloat(
  ...
  $us.WTabs.prototype = {
    ...
    setBarPosition: function (index, animated) {
      ...
      if (this.curLayout === "hor") {
        ...
        css = { top: this.tabTops[index], height: this.tabHeights[index] };
      }
      if (!animated) {
...
(function ($, undefined) {
  ...
  $.extend($us.wVideo.prototype, {
    _confirm: function () {
      var self = this;
      if (
        $("input[name^=" + self.cookieName + "]:checked", self.$container)
          .length
      ) {
      ...
      if (self.isWithOverlay) {
        ...
        self.$videoH
          .html(
            $ush.base64Decode(
              "" + $('script[type="text/template"]', self.$container).text()
            )
          )
    ...
    insertPlayer: function () {
      ...
      if (
        data.player_api &&
        !$('script[src="' + data.player_api + '"]', document.head).length
      ) {
